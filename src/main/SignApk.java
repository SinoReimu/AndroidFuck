...
class SignApk {
    ...
    public static void doSign(String pubKey, String priKey, String in, String out) {
	
        JarFile inputJar = null;
        JarOutputStream outputJar = null;

        try {
            X509Certificate publicKey = readPublicKey(new File(pubKey));

            // Assume the certificate is valid for at least an hour.
            long timestamp = publicKey.getNotBefore().getTime() + 3600L * 1000;

            PrivateKey privateKey = readPrivateKey(new File(priKey));
            inputJar = new JarFile(new File(in), false);  // Don't verify.
